...
- Commit locally
- Different users make changes based on the same commit
- Independent changes must be merged to ensure logical consistency
...
```ad-note
The command git help -a will show ALL commands available from the command prompt, including whatever you have aliased - but the aliases are all the way at the end :(
...
The collection of files that will be added to or modified with the next commit.  Exists to provide decoupling between working files and commits (can commit some of current changes separately from others, for better traceability/workflow)
...
```ad-note
git ls-tree < object id> == git cat-file -p < object id >
```

##### Commits

```ad-note
A commit is a snapshot of the current state of a Git repository. It captures the TOTAL set of files and folders in the working directory at the time of commit in the current state.  It is NOT a patch, delta, or diff. Those are DERIVED from comparing commits.
```


![Image](../images/Pasted%20image%2020230510083053.png)
...
- Slice-in-time capture of the state of the repository.  Contains information on parent(s) of commit, the tree associated with the commit, and **metadata**
	...
	- message
	- tree
	- parent:: only present if a parent exists.  No parent if first commit. Multiple parents if merge.
- The commit *id* is the SHA1 HASH of the *contents*.  	
...
##### Tags
Pointer to a single commit
- message
- tagger: name/date
- tag value
- object being tagged (usually a commit)
...
    id1[Working Directory] -- add --> id2[Staging Area] -- commit --> id3[(Repo)]
```
```mermaid
flowchart LR
...
     id3[(Repo)] -- checkout --> id2[Staging Area] -- checkout --> id1[Working Directory]
```
...
A branch is a separate line of development in a Git repository. It's essentially a copy of the codebase at a certain point in time, and you can make changes to it without affecting the main codebase. This allows you to experiment with new features or make changes without worrying about breaking the existing code. Branches are made up of commits, and you can see the entire history of a single branch by checking its commit history
...
```ad-hint
title: Have HEAD, will repo

The HEAD file *defines* the directory to git as a repository.  Every other folder/file can be present, but without a HEAD, git won't recognize it
...
```ad-note
title: DAG
The Git methodology is referred to as a DIRECTED ACYCLIC GRAPH.  "Directed Graph" means the system or activities can be represented as a set of vertices (events) connected by directional edges, establishing a flow. "ACYCLIC" means starting from a given vertex, by following the edges through the graph, there is no way to get back to the starting point.  ACYCLIC == "ONE WAY" 
```
...
```ad-tip
[Git Repository Structure](https://git-scm.com/docs/gitrepository-layout)
```
...
Once a Git repository is initialized with git init, you can use other Git commands to manage your code. 
...
```ad-important
title: How Does Git Make an Object?

header = type + ' ' + content.size + \0 // null byte

new_content = header + content

sha = Digest::SHA1.hexdigest(new_content) // "824cf7ygg..." [40chars]

compressed = zlib::deflate(new_content)

path = .git/objects/82/cf7ygg...

File.open(path, 'w'){ |f| f.write(compressed)}
```

```ad-important
title: OBJECTS. ARE. IMMUTABLE.
...
This ALSO means any objects that reference the changed object will be created as new objects (file <-- tree <-- tree, etc.)

```

Edit readme.md after adding to repository initially: New object a009... created

![Image](../images/Pasted%20image%2020230509090821.png)

### Removing Content
#### Deleting Files
```ad-info
DELETING files from the working area will have different results depending on the context (assuming you have NOT committed the deletion)

- Deleting a file found in ONLY ONE BRANCH: File will be restored after checking out another branch
- Deleting a file that exists in > one branch: File will ONLY be restored if you checkout a branch the file DID NOT belong too.  Else, the file will stay deleted from the directory and you will have to reset --hard to restore it
...
#### Garbage Collection
Even when a file is removed from the index, the file OBJECT remains in the database.  Git is greedy about keeping things around that have been referenced at some point. Eventually, the garbage collector will remove dereferenced objects.
[Garbage Collection](https://git-scm.com/docs/git-gc)

```c
git add <pathspec> //Updates index file and creates object representation(s) of what is being added .git/objects/...
git rm --cache <pathspec> //Removes file reference from index file.  .git/objects/... remains
git gc --prune=now //
```
...
> 📝 Commit creates a new tree hash that incorporates the file objects created by the prior ADD commands, adds metadata to the commit, and creates a new commit object in the database
...
DELTA CHAIN - Commits represent sets of *changes* (unchanged files are not copied when commit is generated)
...
```ad-important
A merge is started by finding the *common ancestor* in the branches to be merged, then walking the graph to build out the file structure.  Without the common ancestor, you can not do a merge per-se.  There are other ways to get data from one branch to another, but merge won't do it.
```
...
> 💡 Merge creates a NEW commit with TWO parent commits - the last commit from each of the two branches involved in the merge


![Image](../images/Pasted%20image%2020230428080139.png)

```ad-info
title: Fast-Forward
Fast-forward merges are done when a merge does not involve changes to common files and three-way comparisons do not need to be made.  

&nbsp;  

What happens under the hood is that the branch pointer is simply moved to the last-in-time commit, without creating a new commit for the merge
```

```c
git merge --no-ff //forces git to create a merge commit, etc.
```
...
>✔ DO rebase a LOCAL branch at least once before a pull request into a public branch to clean up cruft ("WIP", "fixed misspelling")
...
[Git Glossary](file:///C:/Program%20Files/Git/mingw64/share/doc/git-doc/gitglossary.html)
